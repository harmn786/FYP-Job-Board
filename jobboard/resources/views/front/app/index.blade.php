...
@section('content')
    <div class="wraper">
        ...
        <div class="col-12 col-xl-8 pt-5">
            ...
            <div class="banner-btn mt-5"><a href="{{ route('explorejobs') }}" class="btn btn-success mb-4 mb-sm-0">Explore Now</a></div>
        </div>
    </div>
    </div>
</div>
...
<div class="col-lg-3 mb-4">
    <div class="card mt-4 shadow">
        <div class="card-body">
            <h5 class="card-title">{{ $featured_job->title }}</h5>
            <div class="border-bottom" style="border-color:green; "></div>
            <p class="card-text">Salary: ${{ $featured_job->salary }}</p>
            <p class="card-text">Type: {{ $featured_job->type }}</p>
            <p class="card-text">Location: {{ $featured_job->location }}</p>
            <p class="card-text">Posted by: {{ $featured_job->employer->name }}</p>
            <p class="card-text">Application Deadline: {{\Carbon\Carbon::parse( $featured_job->application_deadline)->format('d M, Y') }}</p>
            <div class="border-bottom my-2"></div>
            <a href="{{ route('jobs.jobDetail', $featured_job->id) }}" class="btn btn-success">View Details</a>
            {{-- Add other job details as needed --}}
        </div>
    </div>
    </div>
